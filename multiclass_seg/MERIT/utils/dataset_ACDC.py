...
class ACDCdataset(Dataset):
    ...
    def __getitem__(self, idx):
        # --- START: DEFINITIVE CORRECTED CODE ---
        
        # 1. Get the base name from train.txt (e.g., '2abd004c...')
        slice_name_without_ext = self.sample_list[idx].strip('\n')

        # 2. Manually add the '.npz' extension
        slice_name = slice_name_without_ext + ".npz"

        # 3. Construct the correct path (without the 'split' subfolder)
        data_path = os.path.join(self.data_dir, slice_name)

        data = np.load(data_path)

        # 4. Use the correct key 'image' to load the data
        image, label = data['image'], data['label']
        
        # --- END: DEFINITIVE CORRECTED CODE ---

        sample = {'image': image, 'label': label}
        if self.transform and self.split == "train":
            sample = self.transform(sample)
        # Note: 'case_name' should probably be without the extension
        sample['case_name'] = slice_name_without_ext
        return sample
