...
def val_single_volume(image, label, net, classes, patch_size=[256, 256],
                      test_save_path=None, case=None, z_spacing=1, use_dual=False):
        """
        2D-compatible validation function for MIST multiclass setup.
        Works for both dual and non-dual supervision.
        """
        image = image.squeeze(0).cpu().detach().numpy()
        label = label.squeeze(0).cpu().detach().numpy()

        # Handle 2D or [C, H, W] format
        if len(image.shape) == 3:
            # If image is [C,H,W], collapse to [H,W]
            if image.shape[0] in [1, 3]:
                image = np.mean(image, axis=0) if image.shape[0] > 1 else image[0]
        ...
        with torch.no_grad():
            if use_dual:
                all_p = net(input_tensor)
                P_fg = all_p[:4]
                # We don't need P_bg for inference
                outputs = 0.0
                # Just sum the outputs from the main foreground heads
                for idx in range(len(P_fg)):
                    outputs += P_fg[idx] # CHANGE THIS LINE
            else:
                P = net(input_tensor)
                outputs = 0.0
                for idx in range(len(P)):
            ...
            unique_gt = np.unique(label)
            print(f"[DEBUG] Predicted unique labels: {unique_pred}, Ground truth unique labels: {unique_gt}")
