...
# # runs segmentation, and visualizes the results with bounding circles and mask overlays.
# # This version introduces several key optimizations in the post-processing pipeline to
...
#         # =====================================================================================
#         # POST-PROCESSING (FULLY OPTIMIZED V2 - MODIFIED FOR MASK & CIRCLE)
#         # =====================================================================================
#         posst = time.time()
...
#             # 6. THRESHOLDING FIRST: Only proceed if confidence is high.
...
#                 # 8. Find contours on the CLEANED mask.
...
#                 # --- NEW (START): ADD MASK OVERLAY ---
#                 # Resize the clean 256x256 mask to the original frame size
#                 resized_mask = cv2.resize(clean_mask, (original_width, original_height), interpolation=cv2.INTER_NEAREST)

#                 # Create a solid color image based on the mask
#                 color_overlay = np.zeros_like(frame, dtype=np.uint8)
#                 color_overlay[resized_mask > 0] = color

#                 # Blend the color overlay with the original frame to create a transparent mask effect
#                 alpha = 1.0  # Weight of the original frame
#                 beta = 0.4   # Weight of the mask overlay (controls transparency)
#                 frame = cv2.addWeighted(frame, alpha, color_overlay, beta, 0)
#                 # --- NEW (END): ADD MASK OVERLAY ---

#                 # 9. Loop through the filtered contours and draw them
#                 for contour in contours:
#                     # --- MODIFIED (START): USE CIRCLE INSTEAD OF RECTANGLE ---
#                     # Calculate the minimum enclosing circle for the contour
#                     (x, y), radius = cv2.minEnclosingCircle(contour)

#                     # Only proceed if the detected radius is of a reasonable size
#                     if radius > 5:
#                         # Scale the circle's center and radius back to the original frame size
#                         orig_center_x = int(x * scale_x)
#                         orig_center_y = int(y * scale_y)
#                         orig_radius = int(radius * scale_x) # Use one scale factor for a more circular shape
...
#                         # If text would go off-screen at the top, place it below the circle instead
#                         if text_y < 15:
#                             text_y = orig_center_y + orig_radius + 20

#                         cv2.putText(frame, display_text, (orig_center_x - orig_radius, text_y), cv2.FONT_HERSHEY_SIMPLEX, FONT_SCALE, color, BOX_THICKNESS)
#                     # --- MODIFIED (END): USE CIRCLE INSTEAD OF RECTANGLE ---
