...
class EMCADNet(nn.Module):
    def __init__(self, num_classes=1, kernel_sizes=[1,3,5], expansion_factor=2, dw_parallel=True, add=True, lgag_ks=3, activation='relu', encoder='pvt_v2_b2', pretrain=True, pretrained_path_base=None,**kwargs):
        ...
        # Get the directory where this networks.py file is located. This is the key to a robust path.
        _this_dir = os.path.dirname(os.path.abspath(__file__))
        _pretrained_path_base = os.path.join(_this_dir, '..', 'pretrained_pth')

        # backbone network initialization with pretrained weight
        if encoder == 'pvt_v2_b0':
            self.backbone = pvt_v2_b0()
            path = os.path.join(_pretrained_path_base, 'pvt', 'pvt_v2_b0.pth')
            channels=[256, 160, 64, 32]
        elif encoder == 'pvt_v2_b1':
            self.backbone = pvt_v2_b1()
            path = os.path.join(_pretrained_path_base, 'pvt', 'pvt_v2_b1.pth')
            channels=[512, 320, 128, 64]
        elif encoder == 'pvt_v2_b2':
            self.backbone = pvt_v2_b2()
            path = os.path.join(_pretrained_path_base, 'pvt', 'pvt_v2_b2.pth')
            channels=[512, 320, 128, 64]
        elif encoder == 'pvt_v2_b3':
            self.backbone = pvt_v2_b3()
            path = os.path.join(_pretrained_path_base, 'pvt', 'pvt_v2_b3.pth')
            channels=[512, 320, 128, 64]
        elif encoder == 'pvt_v2_b4':
            self.backbone = pvt_v2_b4()
            path = os.path.join(_pretrained_path_base, 'pvt', 'pvt_v2_b4.pth')
            channels=[512, 320, 128, 64]
        elif encoder == 'pvt_v2_b5':
            self.backbone = pvt_v2_b5()
            path = os.path.join(_pretrained_path_base, 'pvt', 'pvt_v2_b5.pth')
            channels=[512, 320, 128, 64]
        ...
        if pretrain==True and 'pvt_v2' in encoder:
            print(f"Attempting to load pretrained weights from: {path}")
